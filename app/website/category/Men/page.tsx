...
export default function MensProductsPage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      const mensProducts = res.data.products.filter(
        (p: ProductType) => p.category?.title === "Mens"
      );
  ...
  }, [products, selectedTypes, selectedColors, priceRange]);
    ...
    <div className="p-6 grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
      ...
      <div>
        <h1 className="text-3xl font-bold mb-6">
          Mens Products ({filteredProducts.length})
        </h1>
...
function FilterSection({
  ...
  return (
    <div className="mb-4">
      <p className="font-semibold mb-2 text-indigo-600">{title}</p>
      {values.map((v: string) => (
